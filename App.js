...
const colors = { page: '#F8F7FC', lavender: '#D1CFE8', purple: '#705BAE', purpleDark: '#56438B', ink: '#24212C', muted: '#817B90', line: '#E9E6F0', white: '#FFFFFF', softPurple: '#EFECF8', danger: '#C75F6A' };
...
export default function App() {
  ...
  const signOut = () => Alert.alert('Log out?', 'You can sign in again whenever you’re ready.', [
    ...
    { text: 'Log out', style: 'destructive', onPress: async () => { await session.clear(); setData(initialData); setToken(null); setPhase('signedOut'); } },
  ]);

  if (phase === 'loading') return <SystemScreen icon="sync-outline" title="Loading Secret" text="Loading your space…" />;
  if (phase === 'offline') return <SystemScreen icon="cloud-offline-outline" title="We couldn’t connect" text={error || 'Check your connection and try again.'} onPrimary={restore} primaryLabel="Try again" />;
  ...
  return (
    <SafeAreaView style={styles.app}>
      ...
      {screen === 'home' && <HomeScreen data={data} onNavigate={setScreen} onOpenPost={setSelectedPost} onOpenProfile={openProfile} onCreatePost={createPost} onFavorite={favorite} onSettings={() => setScreen('settings')} onRefresh={reload} />}
      ...
      {screen === 'settings' && <SettingsScreen currentUser={data.currentUser} preferences={data.preferences} onUpdate={updatePreferences} onBack={() => setScreen('home')} onSignOut={signOut} />}
      {screen === 'profile' && <ProfileScreen profile={profile} currentUser={data.currentUser} onBack={() => setScreen('home')} onFollow={follow} onOpenPost={setSelectedPost} onFavorite={favorite} />}
...
function AuthScreen({ error, onLogin, onRegister }) {
  ...
  const valid = register ? username.trim() && email.includes('@') && password.length >= 4 && password === confirm : username.trim() && password;
    ...
    <KeyboardAvoidingView style={styles.authWrap} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.authScroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.authCard}>
          ...
          {register && <Input label="Email" value={email} onChangeText={setEmail} placeholder="you@example.com" autoCapitalize="none" keyboardType="email-address" />}
...
function HomeScreen({ data, onNavigate, onOpenPost, onOpenProfile, onCreatePost, onFavorite, onSettings, onRefresh }) {
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <View style={styles.search}><Icon name="search-outline" color={colors.muted} size={19} /><TextInput value={search} onChangeText={setSearch} style={styles.searchInput} placeholder="Search people or questions" placeholderTextColor={colors.muted} /></View>
        <View style={styles.composer}><Pressable onPress={() => onOpenProfile(data.currentUser)}><Avatar person={data.currentUser} size={45} /></Pressable><View style={styles.composerBody}><TextInput value={draft} onChangeText={setDraft} style={styles.composerInput} multiline maxLength={220} placeholder="Ask what’s on your mind…" placeholderTextColor={colors.muted} /><View style={styles.composerFooter}><Text style={styles.mutedSmall}>{draft.length}/220</Text><Pressable disabled={!draft.trim()} onPress={publish} style={[styles.smallButton, !draft.trim() && styles.disabled]}><Text style={styles.smallButtonText}>Post</Text></Pressable></View></View></View>
        <TitleRow title="Your feed" onRefresh={onRefresh} />
        {posts.length ? posts.map((post) => <PostCard key={String(post.id)} post={post} onOpen={() => onOpenPost(post)} onFavorite={() => onFavorite(post.id)} onOpenProfile={() => onOpenProfile(post.author || post.user)} />) : <Empty icon="chatbubble-ellipses-outline" text={query ? 'No conversations match your search.' : 'Your feed is waiting for its first conversation.'} />}
      </ScrollView>
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <TitleRow title="Trending now" onRefresh={onRefresh} />
...
function SettingsScreen({ currentUser, preferences, onUpdate, onBack, onSignOut }) {
  const rows = [
    ['emailNotifications', 'mail-outline', 'Email notifications', 'News and activity'],
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.settingsProfile}><Avatar person={currentUser} size={58} /><View style={[styles.flex, styles.personCopy]}><Text style={styles.postName}>{nameOf(currentUser)}</Text><Text style={styles.mutedSmall}>{currentUser?.username ? `@${currentUser.username}` : currentUser?.email || ''}</Text></View><Icon name="chevron-forward" color={colors.muted} size={21} /></View>
        <Text style={styles.groupLabel}>PREFERENCES</Text>
        <View style={styles.card}>{rows.map(([key, icon, title, detail], index) => <Pressable key={key} onPress={() => onUpdate({ [key]: !preferences[key] })} style={[styles.setting, index < rows.length - 1 && styles.divider]}><View style={styles.settingIcon}><Icon name={icon} color={colors.purple} size={16} /></View><View style={[styles.flex, styles.personCopy]}><Text style={styles.settingTitle}>{title}</Text><Text style={styles.mutedSmall}>{detail}</Text></View><Switch value={Boolean(preferences[key])} onValueChange={(value) => onUpdate({ [key]: value })} trackColor={{ false: '#DED9E8', true: '#A99AD2' }} thumbColor={colors.white} /></Pressable>)}</View>
        <Text style={styles.groupLabel}>SUPPORT</Text>
        <View style={styles.card}><StaticSetting icon="help-circle-outline" title="Help center" detail="Get answers and contact support" /><StaticSetting icon="information-circle-outline" title="About Secret" detail="Learn about the community" /></View>
...
function ProfileScreen({ profile, currentUser, onBack, onFollow, onOpenPost, onFavorite }) {
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.tabs}>{['Posts', 'Answers', 'Favorites'].map((item) => <Pressable key={item} onPress={() => setTab(item)} style={[styles.tab, tab === item && styles.activeTab]}><Text style={[styles.tabText, tab === item && styles.activeTabText]}>{item}</Text></Pressable>)}</View>
        {tabPosts.length ? tabPosts.map((post) => <PostCard key={String(post.id)} post={post} onOpen={() => onOpenPost(post)} onFavorite={() => onFavorite(post.id)} onOpenProfile={() => {}} />) : <Empty icon={tab === 'Posts' ? 'chatbubble-ellipses-outline' : 'folder-open-outline'} text={`No ${tab.toLowerCase()} to show yet.`} />}
      </ScrollView>
    </View>
  );
}
...
function Avatar({ person, size }) {
  ...
  return <View style={[styles.fallbackAvatar, { width: size, height: size, borderRadius: size / 2 }]}><Text style={{ color: colors.purpleDark, fontWeight: '800', fontSize: Math.max(10, size * 0.34) }}>{initials || '?'}</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  inputGroup: { marginBottom: 16 }, inputLabel: { color: colors.ink, fontSize: 12, fontWeight: '700', marginBottom: 7 }, inputShell: { minHeight: 49, borderWidth: 1, borderColor: '#E0DCE9', backgroundColor: '#FCFBFE', borderRadius: 13, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 13 }, input: { flex: 1, color: colors.ink, fontSize: 15, paddingVertical: 10 }, inputAction: { color: colors.purple, fontSize: 12, fontWeight: '800', paddingLeft: 8 },
  primaryButton: { height: 52, borderRadius: 14, backgroundColor: colors.purple, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', marginTop: 6 }, primaryButtonText: { color: colors.white, fontSize: 15, fontWeight: '800' }, buttonEnd: { position: 'absolute', right: 18 }, disabled: { opacity: 0.58 }, inlineError: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF7F7', borderRadius: 10, padding: 10, marginBottom: 12 }, errorText: { flex: 1, color: colors.danger, fontSize: 12, marginLeft: 6, lineHeight: 17 },
  ...
  settingsProfile: { backgroundColor: colors.lavender, borderRadius: 18, minHeight: 88, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, marginBottom: 25 }, groupLabel: { color: colors.muted, fontSize: 10, fontWeight: '800', letterSpacing: 1.1, marginBottom: 8, marginLeft: 3 }, setting: { minHeight: 70, flexDirection: 'row', alignItems: 'center' }, settingIcon: { width: 33, height: 33, borderRadius: 10, backgroundColor: colors.softPurple, alignItems: 'center', justifyContent: 'center' }, settingTitle: { color: colors.ink, fontSize: 13, fontWeight: '800' }, logoutButton: { height: 51, borderWidth: 1, borderColor: '#F0CED2', backgroundColor: '#FFF9F9', borderRadius: 14, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }, logoutText: { color: colors.danger, fontSize: 14, fontWeight: '800' },
  profileHero: { flexDirection: 'row', alignItems: 'center' }, profileCopy: { marginLeft: 14 }, profileName: { color: colors.ink, fontSize: 21, letterSpacing: -0.4, fontWeight: '800' }, followButton: { height: 35, paddingHorizontal: 16, borderRadius: 18, backgroundColor: colors.purple, justifyContent: 'center', alignItems: 'center' }, followingButton: { backgroundColor: colors.softPurple, borderWidth: 1, borderColor: '#D8D2E6' }, followText: { color: colors.white, fontSize: 12, fontWeight: '800' }, followingText: { color: colors.purpleDark }, bio: { color: '#5E5867', fontSize: 13, lineHeight: 19, marginTop: 20 }, stats: { height: 74, borderRadius: 16, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, marginTop: 21, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around' }, stat: { alignItems: 'center', flex: 1 }, statValue: { color: colors.ink, fontSize: 18, fontWeight: '800' }, statLine: { height: 29, width: 1, backgroundColor: colors.line }, tabs: { flexDirection: 'row', borderBottomWidth: 1, borderColor: colors.line, marginTop: 26, marginBottom: 15 }, tab: { flex: 1, height: 38, alignItems: 'center', justifyContent: 'center', borderBottomWidth: 2, borderColor: 'transparent' }, activeTab: { borderColor: colors.purple }, tabText: { color: colors.muted, fontSize: 12, fontWeight: '700' }, activeTabText: { color: colors.purple, fontWeight: '800' },
  ...
  overlay: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(35, 29, 48, 0.38)' }, dismiss: { flex: 1 }, sheet: { height: '75%', minHeight: 420, backgroundColor: colors.white, borderTopLeftRadius: 27, borderTopRightRadius: 27, paddingHorizontal: 18 }, handle: { alignSelf: 'center', width: 38, height: 4, borderRadius: 3, backgroundColor: '#D7D1E1', marginTop: 9 }, sheetHeader: { height: 57, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, sheetTitle: { color: colors.ink, fontSize: 18, fontWeight: '800' }, close: { width: 31, height: 31, borderRadius: 16, backgroundColor: colors.softPurple, justifyContent: 'center', alignItems: 'center' }, question: { flexDirection: 'row', backgroundColor: '#F9F8FC', borderRadius: 14, padding: 12, marginBottom: 4 }, questionCopy: { marginLeft: 10 }, questionText: { color: '#5F5969', fontSize: 12, lineHeight: 17, fontWeight: '600', marginTop: 4 }, responses: { flex: 1 }, responsesContent: { paddingVertical: 6 }, response: { flexDirection: 'row', paddingVertical: 12 }, responseCopy: { marginLeft: 10 }, responseTop: { flexDirection: 'row', alignItems: 'baseline', gap: 6 }, responseBody: { color: '#57515F', fontSize: 13, lineHeight: 18, marginTop: 4 }, replyInput: { minHeight: 64, paddingVertical: 9, borderTopWidth: 1, borderColor: colors.line, flexDirection: 'row', alignItems: 'center' }, replyTextInput: { flex: 1, minHeight: 39, maxHeight: 75, borderRadius: 17, backgroundColor: '#F7F6FA', color: colors.ink, marginLeft: 9, paddingHorizontal: 13, paddingVertical: 9, fontSize: 13 }, send: { width: 35, height: 35, borderRadius: 18, backgroundColor: colors.purple, marginLeft: 8, alignItems: 'center', justifyContent: 'center' },
