...
export function HomeScreen({ data, onNavigate, onOpenPost, onOpenProfile, onCreatePost, onPickPostImage, onFavorite, onEditPost, onDeletePost, onSettings, onRefresh, styles, colors }) {
  ...
  return <View style={styles.screen}>
    ...
    <FlatList
      ...
      ListHeaderComponent={<><View style={styles.search}><Icon name="search-outline" color={colors.muted} size={19} /><TextInput value={search} onChangeText={setSearch} style={styles.searchInput} placeholder="Search people or questions" placeholderTextColor={colors.muted} /></View><PostComposer currentUser={data.currentUser} onOpenProfile={() => onOpenProfile(data.currentUser)} onCreatePost={onCreatePost} onPickPhoto={onPickPostImage} styles={styles} colors={colors} /><TitleRow title="Your feed" onRefresh={handleRefresh} styles={styles} colors={colors} /></>}
